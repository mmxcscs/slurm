...
static void _shutdown_x11(int signal)
{
	...
	if (session) {
		libssh2_session_disconnect(session,
					   "Client disconnecting due to shutdown.");
...
extern int setup_x11_forward(stepd_step_rec_t *job, int *display)
{
	...
	if (strstr(user_auth_methods, "hostbased")) {
		...
		hostauth_failed = libssh2_userauth_hostbased_fromfile(session,
								      job->user_name,
								      hostkey_pub,
								      hostkey_priv,
								      NULL,
		...
		if (hostauth_failed) {
			char *libssh2_error;
			libssh2_session_last_error(session, &libssh2_error, NULL, 0);
			error("hostkey authentication failed: %s", libssh2_error);
		} else {
	 ...
	 */
	if (hostauth_failed && libssh2_userauth_publickey_fromfile(session, job->user_name, keypub, keypriv, NULL)) {
	...
	*display = port - X11_TCP_PORT_OFFSET;
	...
	/*
	 * Connection handling threads are still running. Return now to signal that
	 * the forwarding code setup has completed successfully, and let steps
	 * needing X11 forwarding service launch now.
	 */
...
shutdown:
	...
	xfree(keypriv);
	close(listen_socket);
	libssh2_session_disconnect(session, "Client disconnecting due to error.");
	libssh2_session_free(session);
	close(ssh_socket);
	libssh2_exit();
...
void *_keepalive_engine(void *x)
{
	...
	while (running) {
		debug("x11 forwarding - sending keepalive message on ssh channel");
...
void *_accept_engine(void *x)
{
	...
	while (true) {
		...
		if ((ci->socket = slurm_accept_msg_conn(listen_socket, &sin)) == -1) {
			xfree(ci);
			error("accept call failed: %m");
			goto shutdown;
		}
...
void *_handle_channel(void *x) {
	...
	while (true) {
		...
		/* read on socket is blocking, so make sure it has data for us */
